...
// verify if the current logged in user is of the type game manager, middleware
// this should be preceded by the verifyToken middleware which checks whether the user is authenticated
exports.verifyIsGameManager = (req, res, next) => {
  if (
    !req.userInfo.user.role ||
    parseInt(req.userInfo.user.role) !== UserType.GAME_MANAGER
  )
    return res.status(403).json({
      response:
        "Authorization failed, game manager access is required!",
    });
